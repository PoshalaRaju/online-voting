...
def log_server(root, frame1, client_socket, voter_ID, password):
    ...
    client_socket.send(message.encode())  # Send voter ID and password to the server

    # Authentication message received from the server
    auth_message = client_socket.recv(1024).decode()

    print("Authentication Message from Server:", auth_message)

    if auth_message == "Authenticate":
        # If authentication is successful, proceed to voting page
        votingPg(root, frame1, client_socket)

    elif auth_message == "VoteCasted":
        message = "Vote has Already been Cast"
        failed_return(root, frame1, client_socket, message)

    elif auth_message == "InvalidVoter":
...
def voterLogin(root, frame1):
    ...
    Label(frame1, text="Voter Login", font=("Helvetica", 18, "bold")).grid(
        row=0, column=2, rowspan=1
    )
    Label(frame1, text="").grid(row=1, column=0)
    Label(frame1, text="Voter ID:      ", anchor="e", justify=LEFT).grid(
        row=2, column=0
    )
