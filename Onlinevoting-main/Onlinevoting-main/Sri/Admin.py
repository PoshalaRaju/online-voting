...
def AdminHome(root, frame1, frame3):
    ...
    Button(frame3, text="Admin", command=lambda: AdminHome(root, frame1, frame3)).grid(
        row=1, column=0
    )
...
def log_admin(root, frame1, admin_ID, password):
    ...
    if admin_ID == "SPEC" and password == "Spec":
...
def AdmLogin(root, frame1):
    ...
    Label(frame1, text="").grid(row=1, column=0)
    Label(frame1, text="Admin ID:      ", anchor="e", justify=LEFT).grid(
        row=2, column=0
    )
    Label(frame1, text="Password:       ", anchor="e", justify=LEFT).grid(
        row=3, column=0
    )
    ...
    sub = Button(
        ...
        text="Login",
        width=10,
        command=lambda: log_admin(root, frame1, admin_ID.get(), password.get()),
    )
    Label(frame1, text="").grid(row=4, column=0)
