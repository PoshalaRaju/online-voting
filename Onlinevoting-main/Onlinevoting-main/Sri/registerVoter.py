...
def reg_server(root,frame1,name,gender,aadhar,city,passw):
    if(passw=='' or passw==' '):
        msg = Message(frame1, text="Error: Missing Fileds", width=500)
        msg.grid(row = 10, column = 0, columnspan = 5)
        return -1

    vid = df.taking_data_voter(name, gender, aadhar, city, passw)
...
def Register(root,frame1):
    ...
    reg = Button(frame1, text="Register", command = lambda: reg_server(root, frame1, name.get(), Gender.get(), aadhar.get(), city.get(), password.get()), width=10)
